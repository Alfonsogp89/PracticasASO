...
void bloquea_signal(int signal)
{
    ...
    sigemptyset(&blocked_signals); // Inicializamos a 0 para asegurarnos que no hay nada corrupto
    sigaddset(&blocked_signals, signal); // Añadimos señal que queremos bloquear
    if (sigprocmask(SIG_BLOCK, &blocked_signals, NULL) == -1) { //Llamada al sistema para bloquear señal
        perror("sigprocmask()");
        exit(EXIT_FAILURE);
    }
}
...
{
    struct sigaction sa;
    memset(&sa, 0, sizeof(struct sigaction)); // Inicialzar a 0 la estructra SIEMPRE
